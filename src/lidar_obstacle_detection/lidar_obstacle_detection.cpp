...
ObstacleDetectionNode::ObstacleDetectionNode(ros::NodeHandle node_handle, float angle_thrsh)
	...
	node_handle.param<float>("/lidar_obstacle_detection_node/obstacle_sector_angle", _obstacle_sector_angle, 0.523598);
    node_handle.param<float>("/lidar_obstacle_detection_node/obstacle_sector_radius", _obstacle_sector_radius, 9);
...
void ObstacleDetectionNode::_filter_dangerous_obstacle_points() {
	...
	for (auto &point : *_obstacle_points) {
		if (cloud_geometry::is_inside_sector(point, _obstacle_sector_angle,
											 _obstacle_sector_radius)) {
